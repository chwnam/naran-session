...
if ( ! class_exists( 'NSESS_Reg_Meta' ) ) {
	/**
	 ...
	 */
	class NSESS_Reg_Meta implements NSESS_Reg {
			...
			if ( isset( $wp_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
				...
				if ( ! isset( static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
					static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] =
						new NSESS_Reg_Meta( $object_type, $meta_key, $args );
				}
			...
			if ( $this->object_type && $this->get_key() ) {
				try {
					if ( $this->args['sanitize_callback'] ) {
						$this->args['sanitize_callback'] = nsess_parse_callback( $this->args['sanitize_callback'] );
					}
					...
					$error->add(
						...
						sprintf(
							...
							nsess_format_callback( $this->args['sanitize_callback'] )
						)
					);
					wp_die( $error );
				}
				...
				try {
					if ( $this->args['auth_callback'] ) {
						$this->args['auth_callback'] = nsess_parse_callback( $this->args['auth_callback'] );
					}
				} catch ( NSESS_Callback_Exception $e ) {
					$error = new WP_Error();
					$error->add(
						...
						sprintf(
							...
							nsess_format_callback( $this->args['auth_callback'] )
						)
					);
					wp_die( $error );
				}
