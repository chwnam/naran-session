...
if ( ! class_exists( 'NSESS_EJS_Queue' ) ) {
	class NSESS_EJS_Queue implements NSESS_Module {
		use NSESS_Template_Impl;
		...
		public function __construct() {
			if ( is_admin() ) {
				if ( ! has_action( 'admin_print_footer_scripts', [ $this, 'do_template' ] ) ) {
					add_action( 'admin_print_footer_scripts', [ $this, 'do_template' ], nsess()->get_priority() );
				}
			} else {
				if ( ! has_action( 'wp_print_footer_scripts', [ $this, 'do_template' ] ) ) {
					add_action( 'wp_print_footer_scripts', [ $this, 'do_template' ], nsess()->get_priority() );
				}
			}
		}
